...
bool video_reader_open(VideoReaderState *state, const char *device_name) {
  ...
  // Open the video input using libavformat (Video4Linux for Linux)
  av_format_ctx = avformat_alloc_context();
  if (!av_format_ctx) {
  ...
  // Use Video4Linux (v4l2) for Linux USB camera input
  ...
  av_dict_set(&options, "framerate", "30", 0);         // Set desired framerate
  av_dict_set(&options, "video_size", "640x480", 0);   // Set video resolution
  av_dict_set(&options, "pixel_format", "yuv420p", 0); // Use YUV format

  // Open the device (assuming device 0, adjust if needed)
  if (avformat_open_input(&av_format_ctx, device_name, av_input_format,
                          &options) != 0) {
    printf("Couldn't open video device\n");
    return false;
  }

  // Find the first valid video stream inside the device
  video_stream_index = -1;
  AVCodecParameters *av_codec_params;
  AVCodec *av_codec;
  for (unsigned int i = 0; i < av_format_ctx->nb_streams; ++i) {
  ...
  if (video_stream_index == -1) {
    printf("Couldn't find valid video stream inside device\n");
    return false;
  }
